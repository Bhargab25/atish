...
class SlotModel extends BaseModel {
    ...
    public $ACCESS_TIME;
	 ...
	 public static function Create(SlotModel $model) {
        $mysqli = Config::OpenDBConnection();
		 $query = "insert into mainslot ({$model->__columns}) values(NULL, $model->SLOT_ID,$model->STATUS,'$model->ACCESS_TIME')";
	...
	public static function update_on_generate(SlotModel $model){
		$mysqli = Config::OpenDBConnection();		
        $query = "update mainslot set STATUS=?,ACCESS_TIME=? where SLOT_ID=? and ID=?";
        $stmt = Config::CreateStatement($mysqli, $query);
	...
	public static function ReadAll() {
        $model = new SlotModel();
        $mysqli = Config::OpenDBConnection();
        $query = "select * from mainslot order by ID DESC";
        $stmt = Config::CreateStatement($mysqli, $query);
        $stmt->bind_result($model->ID, $model->SLOT_ID, $model->STATUS, $model->ACCESS_TIME);
        $stmt->execute();
        $list = array();
        while ($stmt->fetch()) {
            $obj = new SlotModel();
