...
if (isset($_SESSION['userid'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && !empty($_GET['id'])) {
        ...
        try {
            ...
            $sdamount = sdentityModel::ReadSingle($invoice_deatils->c_id)->due_ammount;
            $current_amount = $sdamount - $invoice_deatils->total_amount;
            // Update due amount in sdentity table
            $query4 = "UPDATE sdentity SET `due_ammount` = ? WHERE id = ?";
            $stmt4 = Config::CreateStatement($mysqli, $query4);
            $stmt4->bind_param('ds', $current_amount, $invoice_deatils->c_id);
            $stmt4->execute();
            ...
            // Delete the invoice History
            $query7 = "DELETE FROM invoice_gst_history WHERE entry_id=?";
            $stmt7 = Config::CreateStatement($mysqli, $query7);
            $stmt7->bind_param('s', $id);
            $stmt7->execute();

            if ($stmt7->error) {
                throw new Exception('Error in query 7: ' . $stmt7->error);
            }
